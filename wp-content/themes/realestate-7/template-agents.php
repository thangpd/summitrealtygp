		...
		if ( $ct_agents_search_fields ) :
			...
			foreach ( $ct_agents_search_fields as $field => $value ) {
				...
				switch ( $field ) {
					...
					case 'state' : ?>
                        <div id="state"
                             class="col <?php ct_agents_search_fields_span(); ?> <?php ct_agents_search_fields_first(); ?>">
                            <select name="state">
                                <option value=""><?php _e( 'Select a State', 'contempo' ); ?></option>
								<?php foreach ( $states as $key => $value ) { ?>
                                    <option value="<?php echo esc_html( $key ); ?>"
                                            title="<?php echo htmlspecialchars( $value ); ?>" <?php if ( $ct_state == $key ) {
										echo 'selected';
					...
					case 'city' : ?>
                        <div class="col <?php ct_agents_search_fields_span(); ?> <?php ct_agents_search_fields_first(); ?>">
                            <input type="text" name="city" id="city"
                                   placeholder="<?php _e( 'City', 'contempo' ); ?>" <?php if ( $ct_city != '' ) {
								echo 'value="' . $ct_city . '"';
							} ?> />
                        </div>
						<?php
					...
					case 'zipcode' : ?>
                        <div class="col <?php ct_agents_search_fields_span(); ?> <?php ct_agents_search_fields_first(); ?>">
                            <input type="text" name="zipcode"
                                   placeholder="<?php _e( 'Zipcode', 'contempo' ); ?>" <?php if ( $ct_zipcode != '' ) {
								echo 'value="' . $ct_zipcode . '"';
							} ?> />
                        </div>
						<?php
						break;
...
<?php if ( ! empty( $ct_brokers_search ) ) { ?>
    <form id="agent-live-search" action="" method="post">
        <fieldset>
            <input type="text" class="text-input" id="agent-filter" value=""
                   placeholder="<?php _e( 'Type an agents name, title, business, phone or lic # here to filter the list.', 'contempo' ); ?>"/>
        </fieldset>
    </form>
...
<?php } ?>
    ...
    <ul class="agents-list">
		...
		if ( $ct_agents_per_page != 0 && $total_agents != 0 ) {
            ...
            <div id="overlay" class="contact-modal-<?php echo esc_html( $curauth->ID ); ?> agent-modal">
                <div id="modal">
                    <div id="modal-inner">
                        ...
                        <form id="listingscontact"
                              class="contact-form-<?php echo esc_html( strtolower( $curauth->last_name ) ); ?> formular"
                              method="post">
                            <fieldset class="col span_12">
                                ...
                                <div class="clear"></div>
                                <input type="text" name="name" id="name" class="validate[required] text-input"
                                       placeholder="<?php esc_html_e( 'Name', 'contempo' ); ?>" <?php if ( is_user_logged_in() ) {
									echo 'value="' . $current_user->user_firstname . ' ' . $current_user->user_lastname . '"';
								} ?> />

                                <input type="text" name="email" id="email"
                                       class="validate[required,custom[email]] text-input"
                                       placeholder="<?php esc_html_e( 'Email', 'contempo' ); ?>" <?php if ( is_user_logged_in() ) {
									echo 'value="' . $current_user->user_email . '"';
								} ?> />

                                <input type="text" name="ctphone" id="ctphone" class="text-input"
                                       placeholder="<?php esc_html_e( 'Phone', 'contempo' ); ?>" <?php if ( is_user_logged_in() ) {
									echo 'value="' . $current_user->mobile . '"';
								} ?> />

                                <textarea class="validate[required,length[2,1000]] text-input" name="message"
                                          id="message" rows="6" cols="10"></textarea>
                                ...
                                <input type="submit" name="Submit" value="<?php esc_html_e( 'Submit', 'contempo' ); ?>"
                                       id="submit" class="btn"/>
                            </fieldset>
                            <div class="clear"></div>
                        </form>
		...
		<?php } ?>
            ...
            <!-- Agent -->
            <li id="<?php echo esc_html( strtolower( $curauth->first_name ) ) . '-' . esc_html( strtolower( $curauth->last_name ) ); ?>"
                class="agent <?php echo esc_html( $ct_agent_layout ); ?> <?php if ( $ct_agent_layout == 'agent-grid' ) {
				    echo 'col span_3';
			    } ?>">
                <figure class="col <?php if ( $ct_agent_layout == 'agent-grid' ) {
					echo 'span_12';
				} else {
					echo 'span_3';
				} ?> first">
                    <a href="<?php echo get_author_posts_url( $curauth->ID ); ?>"
                       title="<?php echo esc_html( $curauth->display_name ); ?>">
						<?php if ( $curauth->ct_profile_url ) { ?>
                            <img class="author-img" src="<?php echo esc_html( $curauth->ct_profile_url ); ?>"/>
						<?php } else { ?>
                            <img class="author-img"
                                 src="<?php echo get_template_directory_uri() . '/images/user-default.png'; ?>"/>
						<?php } ?>
                    </a>
                </figure>
                <div class="agent-info col <?php if ( $ct_agent_layout == 'agent-grid' ) {
					echo 'span_12';
				} else {
					echo 'span_9';
				} ?>">
                    <header>
						<?php if ( $ct_agent_layout == 'agent-grid' ) {
							...
							echo '<h3>';
                               ...
                               title="<?php echo esc_html( $curauth->display_name ); ?>"><?php echo esc_html( $curauth->display_name ); ?></a><?php if ( $ct_agent_layout == 'agent-grid' ) {
							echo '</h4>';
						} else {
							echo '</h3>';
						} ?>
						<?php if ( $curauth->city ) { ?><h5 class="muted position marT5 marB5">Serving
                            the <?php echo esc_html( $curauth->city ); ?> Area</h5><?php } ?>
						<?php if ( $curauth->company_name ) { ?><p class="company-name">
                            <strong><?php echo esc_html( $curauth->company_name ); ?></strong></p><?php } ?>
                    </header>

					<?php if ( $ct_agent_layout != 'agent-grid' ) { ?>
                        <div class="agent-bio col span_8 first">
                            <p class="marT20"><?php if ( $curauth->tagline ) { ?><strong
                                        class="tagline"><?php echo esc_html( $curauth->tagline ); ?></strong> <?php } ?><?php $bio = $curauth->description;
								echo nl2br( $bio ); ?></p>
                            <div class="col span_6 first">
								<?php if ( $curauth->brokeragelicense ) { ?><p class="marT10 marB0"><span
                                            class="muted left"><?php _e( 'Broker#', 'contempo' ); ?></span> <span
                                            class="right"><?php echo esc_html( $curauth->brokeragelicense ); ?></span>
                                    </p><?php } ?>
								<?php if ( $curauth->agentlicense ) { ?><p class="marT10 marB0"><span
                                            class="muted left"><?php _e( 'License#', 'contempo' ); ?></span> <span
                                            class="right"><?php echo esc_html( $curauth->agentlicense ); ?></span>
                                    </p><?php } ?>
                            </div>
                            <div class="clear"></div>
                            <ul class="social marT20 marL0">
								<?php if ( $curauth->twitterhandle ) { ?>
                                    <li class="twitter"><a
                                            href="http://twitter.com/#!/<?php echo esc_html( $curauth->twitterhandle ); ?>"
                                            target="_blank"><i class="fa fa-twitter"></i></a></li><?php } ?>
								<?php if ( $curauth->facebookurl ) { ?>
                                    <li class="facebook"><a href="<?php echo esc_html( $curauth->facebookurl ); ?>"
                                                            target="_blank"><i class="fa fa-facebook"></i></a>
                                    </li><?php } ?>
								<?php if ( $curauth->instagramurl ) { ?>
                                    <li class="instagram"><a href="<?php echo esc_url( $curauth->instagramurl ); ?>"
                                                             target="_blank"><i class="fa fa-instagram"></i></a>
                                    </li><?php } ?>
								<?php if ( $curauth->linkedinurl ) { ?>
                                    <li class="linkedin"><a href="<?php echo esc_html( $curauth->linkedinurl ); ?>"
                                                            target="_blank"><i class="fa fa-linkedin"></i></a>
                                    </li><?php } ?>
								<?php if ( $curauth->gplus ) { ?>
                                    <li class="google"><a href="<?php echo esc_html( $curauth->gplus ); ?>"
                                                          target="_blank"><i class="fa fa-google-plus"></i></a>
                                    </li><?php } ?>
								<?php if ( $curauth->youtubeurl ) { ?>
                                    <li class="youtube"><a href="<?php echo esc_url( $curauth->youtubeurl ); ?>"
                    ...
                    <ul class="agent-info col <?php if ( $ct_agent_layout == 'agent-grid' ) {
						echo 'span_12';
					} else {
						echo 'span_4';
					} ?>">
						<?php if ( $curauth->mobile ) { ?>
                            <li class="row"><span class="muted left"><?php ct_phone_svg(); ?></span> <span
                                    class="right"><a
                                        href="tel:<?php echo esc_html( $curauth->mobile ); ?>"><?php echo esc_html( $curauth->mobile ); ?></a></span>
                            </li><?php } ?>
						<?php if ( $curauth->office ) { ?>
                            <li class="row"><span class="muted left"><?php ct_office_svg(); ?></span> <span
                                    class="right"><a
                                        href="tel:<?php echo esc_html( $curauth->office ); ?>"><?php echo esc_html( $curauth->office ); ?></a></span>
                            </li><?php } ?>
						<?php if ( $curauth->fax ) { ?>
                            <li class="row"><span class="muted left"><?php ct_printer_svg(); ?></span> <span
                                    class="right"><a
                                        href="tel:<?php echo esc_html( $curauth->fax ); ?>"><?php echo esc_html( $curauth->fax ); ?></a></span>
                            </li><?php } ?>
						<?php if ( $curauth->user_email ) {
							$email = $curauth->user_email; ?>
                            <li id="email-agent" class="row"><span class="muted left"><?php ct_envelope_svg(); ?></span>
                            <span class="right"><a class="agent-contact-<?php echo esc_html( $curauth->ID ); ?>"
                                                   href="#"><?php echo esc_html( $email ); ?></a></span></li><?php } ?>
						<?php if ( $curauth->user_url ) {
							...
							?>
                            <li class="row"><span class="muted left"><?php ct_globe_svg(); ?></span> <span
                                        class="right"><a
                                            href="<?php echo esc_html( $curauth->user_url ); ?>"><?php echo esc_html( $ct_domain ); ?></a></span>
                            </li>
						<?php } ?>
						<?php if ( $curauth->brokername ) { ?><p class="marB3">
                            <strong><?php echo esc_html( $curauth->brokername ); ?></strong></p><?php } ?>
						<?php if ( $curauth->brokernum ) { ?><p
                                class="marB3"><?php echo esc_html( $curauth->brokernum ); ?></p><?php } ?>
					...
					<?php if ( $ct_agent_layout != 'agent-grid' ) { ?>
                        <div class="view-listings">
                            <a class="btn btn-secondary"
                               href="<?php echo get_author_posts_url( $curauth->ID ); ?>"><?php esc_html_e( 'View Profile', 'contempo' ); ?></a>
                        </div>
					<?php } ?>
                </div>
                <div class="clear"></div>
