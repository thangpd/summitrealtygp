...
function wpfp_remove_favorite_link($post_id) {
    if (wpfp_is_user_can_edit()) {
        ...
        $link = "<a id='rem_$post_id' class='$class' href='?wpfpaction=remove&amp;page=1&amp;postid=". $post_id ."' title='".wpfp_get_option('rem')."' rel='nofollow'><i class=\"fa fa-heart\"></i></a>";
...
function wpfp_clear_list_link() {
    if (wpfp_is_user_can_edit()) {
        ...
        echo "<a class='wpfp-link btn btn-delete' href='?wpfpaction=clear' rel='nofollow'>". wpfp_get_option('clear') . "</a>";
    }
}
